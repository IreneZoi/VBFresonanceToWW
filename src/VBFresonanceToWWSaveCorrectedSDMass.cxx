...
VBFresonanceToWWSaveCorrectedSDMass::VBFresonanceToWWSaveCorrectedSDMass(Context & ctx, 
...
void VBFresonanceToWWSaveCorrectedSDMass::fill(const uhh2::Event & event){
  ...
  //     hist("Eta_both")->Fill(Eta2, weight);
      ...
      cout << " Tjets size " << Tjets.size() << endl;
      ...
      cout << " lorentz vector " << endl;

      for (const auto s1 : jet1.subjets()) {
	cout << " s1 " << endl;
	subjet_sum1 += s1.v4();
	cout << " subjet_sum1 " << subjet_sum1 << endl;
      }
      auto JetSDMass1 = subjet_sum1.M();
      cout << " uncorrected SD Mass 1 " << JetSDMass1 << endl;

      for (const auto s2 : jet2.subjets()) {
	cout << " s2 " << endl;

	subjet_sum2 += s2.v4();
	cout << " subjet_sum2 " << subjet_sum2 << endl;

      }
      auto JetSDMass2 = subjet_sum2.M();
      cout << " uncorrected SD Mass 2 " << JetSDMass2 << endl;
	...
	recoCorr_2 = puppisd_corrRECO_for->Eval( jet2.pt() );
      ...
      JetSDMass2 = JetSDMass2*totalWeight_2;
      hist("CorrectedSoftDropMass_2")->Fill(JetSDMass2, weight);      

      cout << " corrected SD Mass 1 " << JetSDMass1 << endl;
      cout << " corrected SD Mass 2 " << JetSDMass2 << endl;

      cout << " before set SD Mass 1 "<< event.topjets->at(0).softdropmass()  << endl;  
      cout << " before set SD Mass 1 "<<       Tjets[0].softdropmass()  << endl;  
      Tjets[0].set_softdropmass(JetSDMass1);
      cout << " set SD Mass 1 "  << endl;
      cout << " get SD Mass 1 "<< event.topjets->at(0).softdropmass()  << endl;

      Tjets[1].set_softdropmass(JetSDMass2);

      cout << " set SD Mass 2 "  << endl;
