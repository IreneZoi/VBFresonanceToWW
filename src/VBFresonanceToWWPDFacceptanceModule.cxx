...
for(int i = 0; i < PDFs; i++)
{
  ...
  else
  {
    ...
    h_Wtopjets_passVBF_invM800_de45[i].reset(new VBFresonanceToWW_WTopJetInvMHists(ctx, "Wtopjets_passVBF_invM800_de45_pdf"+pdf));
    h_Wtopjets_failVBF_invM800_de45[i].reset(new VBFresonanceToWW_WTopJetInvMHists(ctx, "Wtopjets_failVBF_invM800_de45_pdf"+pdf));
...
if(PRINT)    cout << " saved original collections " << endl;

for(int i = 0; i < PDFs; i++)
{
  //  float orig_weight = event.genInfo->pdf_scalePDF();

  if(PRINT)    cout << " pdf " << i <<endl;
  event.jets->clear();
  ...
  if(PRINT)    cout << " event weight before pdf weight "<<  event.weight << endl;
  ...
  if(PRINT)    cout << " event weight after pdf " << event.weight <<endl;
  h_Wtopjets_input[i]->fill(event);


  bool pass_cm = common->process(event);
  if(PRINT)    cout << " common " << event.weight <<endl;
  bool muon_selection = muon_sel->passes(event);
  bool electron_selection = electron_sel->passes(event);
  if(PRINT)    cout << " leptons " << endl;
  if(pass_cm && muon_selection && electron_selection)
  {
    if(PRINT)    cout <<  " pass first sel" <<endl;
    sort_by_pt<Jet>(*event.jets);
    sort_by_pt<TopJet>(*event.topjets);

    if(PRINT)    cout <<  " jet ordering" <<endl;
    // JET CLEANING

    if(isMC)
    {
      if(PRINT)    cout <<  " is MC " << isMC<<endl;

      // jetlepton_cleaner->process(event);
      jet_corrector->process(event);
      if(PRINT)    cout <<  " jet corrector" <<endl;
      topjet_corrector->process(event);
      if(PRINT)    cout <<  " topjet corrector" <<endl;
    ...
    if(topjets2_selection)
    {
      ...
      if(invMtopjet_fitselection && topjets_deta_selection && VVMtopjet_selection && tau21topjetHP_selection)
      {
        ...
        if(jets2_selection && vbfetasign_selection &&  invM800jet_selection && vbfeta45_selection)
