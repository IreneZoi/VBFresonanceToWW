...
void VBFresonanceToWWGenDiJetHists::fill(const uhh2::Event & event){
  ...
  const std::vector<Particle> &  jet = event.get(h_genjets);
      ...
      for(unsigned int i = 0; i <jet.size(); i++)
	{
      ...
      /*
      const  std::vector<Particle>  &   eta_jets  =event.get(h_genjets);      
      //    sort_by_eta<Particle>(eta_jets);
      //      sort_by_eta<Particle>();
      //std::sort(eta_jets.at(0).v4(), eta_jets.at(eta_jets.size()-1).v4(), eta_jets.at(0).v4().eta() > eta_jets.at(eta_jets.size()-1).v4().eta());
      ...
      float eta_Phi2;// = Phi2_jets.at(eta_jets.size()-1).v4().phi();
