...
namespace uhh2examples {
   ...
   */
  class VBFresonanceToWWInvertedHSB2Module: public AnalysisModule {
  public:
    

    explicit VBFresonanceToWWInvertedHSB2Module(Context & ctx);
  ...
  private:
    ...
    std::unique_ptr<Selection> VVmass_sel, LOWmass_sel, HIGHmass_sel, HIGHmassSecLeading_sel, VVmassLeading_sel;
  ...
  VBFresonanceToWWInvertedHSB2Module::VBFresonanceToWWInvertedHSB2Module(Context & ctx){
    // In the constructor, the typical tasks are to initialize the
    // member variables, in particular the AnalysisModules such as
    // CommonModules or some cleaner module, Selections and Hists.
    // But you can do more and e.g. access the configuration, as shown below.
    
    cout << "Hello World from VBFresonanceToWWInvertedHSB2Module!" << endl;
    ...
    VVmassLeading_sel.reset(new VVMassLeadingTopjetSelection());// see VBFresonanceToWWSelections
    ...
    HIGHmassSecLeading_sel.reset(new HighMassSecLeadingTopjetSelection(135.0f));// see VBFresonanceToWWSelections
  ...
  bool VBFresonanceToWWInvertedHSB2Module::process(Event & event) {
    ...
    if(PRINT)    cout << "VBFresonanceToWWInvertedHSB2Module: Starting to process event (runid, eventid) = (" << event.run << ", " <<", " << event.event << "); weight = " << event.weight << endl;
   ...
   sort_by_pt<TopJet>(*event.topjets);
    ...
    bool HIGHMLeadingtopjet_selection = HIGHmassSecLeading_sel->passes(event);
    if(PRINT) std::cout<<"SelectionModule - High M leading - Size topjets Collection "<<event.jets->size() <<std::endl;
    bool VVMLeadingtopjet_selection = VVmassLeading_sel->passes(event);
  ...
  // make sure the VBFresonanceToWWInvertedHSB2Module is found by class name. This is ensured by this macro:
  UHH2_REGISTER_ANALYSIS_MODULE(VBFresonanceToWWInvertedHSB2Module)
