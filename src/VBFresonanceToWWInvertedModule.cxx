...
namespace uhh2examples {
  ...
  private:
    ...
    std::unique_ptr<Selection> invM500_invsel, invM1000_invsel, invM1500_invsel, invM2000_invsel;
    std::unique_ptr<Selection> invM2500_invsel, invM3000_invsel, invM3500_invsel, invM4000_invsel;
  ...
  VBFresonanceToWWInvertedModule::VBFresonanceToWWInvertedModule(Context & ctx){
    ...
    invM500_invsel.reset(new invMassVBFjetInvSelection()); // see VBFresonanceToWWSelections
    invM1000_invsel.reset(new invMassVBFjetInvSelection(1000.0f)); // see VBFresonanceToWWSelections
    invM1500_invsel.reset(new invMassVBFjetInvSelection(1500.0f)); // see VBFresonanceToWWSelections
    invM2000_invsel.reset(new invMassVBFjetInvSelection(2000.0f)); // see VBFresonanceToWWSelections
    invM2500_invsel.reset(new invMassVBFjetInvSelection(2500.0f)); // see VBFresonanceToWWSelections
    invM3000_invsel.reset(new invMassVBFjetInvSelection(3000.0f)); // see VBFresonanceToWWSelections
    invM3500_invsel.reset(new invMassVBFjetInvSelection(3500.0f)); // see VBFresonanceToWWSelections
    invM4000_invsel.reset(new invMassVBFjetInvSelection(4000.0f)); // see VBFresonanceToWWSelections
  ...
  bool VBFresonanceToWWInvertedModule::process(Event & event) {
   ...
   sort_by_pt<TopJet>(*event.topjets);
	...
	bool invM500jet_invselection = invM500_invsel->passes(event);
	bool invM1000jet_invselection = invM1000_invsel->passes(event);
	bool invM1500jet_invselection = invM1500_invsel->passes(event);
	bool invM2000jet_invselection = invM2000_invsel->passes(event);
	bool invM2500jet_invselection = invM2500_invsel->passes(event);
	bool invM3000jet_invselection = invM3000_invsel->passes(event);
	bool invM3500jet_invselection = invM3500_invsel->passes(event);
	bool invM4000jet_invselection = invM4000_invsel->passes(event);
	
	if(invM4000jet_invselection)
	  h_Wtopjets_withVBF_invM4000->fill(event);
	
	if(invM3500jet_invselection)
	  h_Wtopjets_withVBF_invM3500->fill(event);
	
	if(invM3000jet_invselection)
	  h_Wtopjets_withVBF_invM3000->fill(event);
	
	if(invM2500jet_invselection)  
	  h_Wtopjets_withVBF_invM2500->fill(event);
	
	if(invM2000jet_invselection)  
	  h_Wtopjets_withVBF_invM2000->fill(event);
	
	if(invM1500jet_invselection)  
	  h_Wtopjets_withVBF_invM1500->fill(event);
	
	if(invM1000jet_invselection)  
	  h_Wtopjets_withVBF_invM1000->fill(event);
	
	if(invM500jet_invselection)  
    ...
    if(jets2_invselection || vbfetasign_invselection || vbfeta_invselection)
