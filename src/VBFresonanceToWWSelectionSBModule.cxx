...
namespace uhh2examples {
   ...
   */
  class VBFresonanceToWWSelectionSBModule: public AnalysisModule {
  public:


    explicit VBFresonanceToWWSelectionSBModule(Context & ctx);
  ...
  private:
    ...
    std::unique_ptr<Selection> VVmass_sel, HIGHmassLeading_sel, VVmassSecLeading_sel, WWmass_sel;
  ...
  VBFresonanceToWWSelectionSBModule::VBFresonanceToWWSelectionSBModule(Context & ctx){
    // In the constructor, the typical tasks are to initialize the
    // member variables, in particular the AnalysisModules such as
    // CommonModules or some cleaner module, Selections and Hists.
    // But you can do more and e.g. access the configuration, as shown below.

    cout << "Hello World from VBFresonanceToWWSelectionSBModule!" << endl;
    ...
    h_Wtopjets_compareTAU.reset(new VBFresonanceToWW_WTopJetHistsCorrectedSDMass(ctx, "Wtopjets_compareTAU"));
  ...
  bool VBFresonanceToWWSelectionSBModule::process(Event & event) {
    ...
    // is thrown away.
    if(PRINT)    cout << "VBFresonanceToWWSelectionSBModule: Starting to process event (runid, eventid) = (" << event.run << ", " <<", " << event.event << "); weight = " << event.weight << endl;
...
if(PRINT) std::cout<<"SelectionSBModule - High M leading - Size topjets Collection "<<event.jets->size() <<std::endl;
bool VVMLeadingtopjet_selection = VVmassSecLeading_sel->passes(event);
if(PRINT) std::cout<<"SelectionSBModule - V M sec leading - Size topjets Collection "<<event.jets->size() <<std::endl;

    if(!HIGHMLeadingtopjet_selection) return false;
    if(PRINT) std::cout<<"SelectionSBModule - high M leading applied - Size topjets Collection "<<event.jets->size() <<std::endl;
  ...
  // make sure the VBFresonanceToWWSelectionSBModule is found by class name. This is ensured by this macro:
  UHH2_REGISTER_ANALYSIS_MODULE(VBFresonanceToWWSelectionSBModule)
